...
function OrderList() {
  ...
  const deleteOrderHandler = async (id) => {
    try {
      await dispatch(deleteOrder(id)); // Dispatch delete action
      console.log("Order ID deleted:", id); // Log the deleted ID
      alert.success("Order Deleted Successfully"); // User-friendly success message
      history.push("/admin/orders"); // Redirect to orders list
    } catch (error) {
      console.error("Error deleting order:", error); // Log the error for debugging
      alert.error("Error deleting order. Please try again."); // User-friendly error message
    }
  };
...
const columns = [
  ...
  },
  {
    ...
    renderCell: (params) => {
      console.log("Order ID:", params.getValue(params.id, "id"));
      return (
        <>
          ...
          {orders.length > 0 && (
          <DeleteIcon className="iconbtn"  onClick={() => deleteOrderHandler(params.getValue(params.id, "id"))}/>
          )}
        </>
      );
    },
